...
`,
  },
  {
    version: 16,
    sql: `
-- Resident agents' durable data (mirrors SQLite v16 — see
-- docs/residents-in-projects-db-plan.md). Resident ids are user-derived
-- slugs, so agents/channels/memories key on (tenant_id, id) — two tenants
-- can both have a "sable". Message/alarm ids are identity-generated
-- (INTEGER, not BIGSERIAL: node-pg returns int8 as string, and the row
-- types share the SQLite number shape).

-- Comment authorship graduates from the 'agent'/'human' enum to free
-- principal strings ('agent:<id>' for resident sessions).
ALTER TABLE ticket_comments DROP CONSTRAINT ticket_comments_author_check;
...
CREATE INDEX idx_resident_agents_tenant ON resident_agents(tenant_id);
...
CREATE TABLE resident_memories (
  ...
  FOREIGN KEY (tenant_id, agent_id) REFERENCES resident_agents(tenant_id, id) ON DELETE CASCADE
);
CREATE INDEX idx_resident_memories_tenant ON resident_memories(tenant_id);
...
CREATE INDEX idx_resident_channels_tenant ON resident_channels(tenant_id);
...
CREATE INDEX idx_resident_messages_channel ON resident_messages(tenant_id, channel, id);
CREATE INDEX idx_resident_messages_tenant ON resident_messages(tenant_id);
...
CREATE INDEX idx_resident_alarms_agent ON resident_alarms(tenant_id, agent_id);
CREATE INDEX idx_resident_alarms_tenant ON resident_alarms(tenant_id);

${['resident_agents', 'resident_memories', 'resident_channels', 'resident_messages', 'resident_alarms']
  .map(rls)
  .join('\n')}

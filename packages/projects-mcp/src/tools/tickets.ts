...
export function registerTicketTools(server: McpServer, repo: IProjectsRepo): void {
  server.tool(
    'get_ticket',
  ...
  server.tool(
    ...
    async ({ query, project_id }) => {
      ...
      const all = project_id ? await repo.listTicketsByProject(project_id) : await repo.listAllTickets();

      const matched = all
        .filter((t) => t.title.toLowerCase().includes(needle) || t.description.toLowerCase().includes(needle))
        .sort((a, b) => (a.updated_at < b.updated_at ? 1 : -1))
        .slice(0, 50);

      // Resolve column labels, caching the column list per project.
      const colsByProject = new Map<string, ColumnRow[]>();
      const labelFor = async (t: TicketRow) => {
        let cols = colsByProject.get(t.project_id);
        if (!cols) {
          cols = await repo.listColumns(t.project_id);
          colsByProject.set(t.project_id, cols);
        }
        return cols.find((c) => c.id === t.column_id)?.label ?? t.column_id;
      };

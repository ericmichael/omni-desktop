...
import { type GroupSummary, selectVictims } from './cleanup';
...
const hoursAgo = (h: number): string => new Date(NOW.getTime() - h * 3600 * 1000).toISOString();
...
describe('selectVictims', () => {
  ...
  it('keeps groups newer than the cutoff', () => {
    const result = selectVictims([group('fresh', { 'omni-launcher': RG, 'omni-created-at': hoursAgo(1) })], {
      launcherTag: RG,
      maxAgeHours: 8,
      now: NOW,
    });
  ...
  it('skips groups not tagged with our launcher (foreign workloads)', () => {
    ...
    expect(result.skipped.map((s) => s.name).sort()).toEqual(['partially-tagged', 'theirs', 'untagged']);
  });
  ...
  it('treats exactly-at-cutoff as still fresh (>cutoff is the predicate, equal is kept)', () => {
    const result = selectVictims([group('edge', { 'omni-launcher': RG, 'omni-created-at': hoursAgo(8) })], {
      launcherTag: RG,
      maxAgeHours: 8,
      now: NOW,
    });

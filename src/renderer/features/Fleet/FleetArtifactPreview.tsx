...
import type { ArtifactFileContent, ArtifactFileEntry, FleetTicketId } from '@/shared/types';

import { fleetApi } from './state';

const MAX_TEXT_PREVIEW_SIZE = 100_000;

type FleetArtifactPreviewProps = {
  ticketId: FleetTicketId;
  selectedFile: ArtifactFileEntry | null;
};

export const FleetArtifactPreview = memo(({ ticketId, selectedFile }: FleetArtifactPreviewProps) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void fleetApi.readArtifact(ticketId, selectedFile.relativePath).then((result) => {
      setContent(result);
      setLoading(false);
    });
  ...
  const handleOpenExternal = useCallback(() => {
    if (selectedFile) {
      void fleetApi.openArtifactExternal(ticketId, selectedFile.relativePath);
    }
  }, [ticketId, selectedFile]);
...
FleetArtifactPreview.displayName = 'FleetArtifactPreview';

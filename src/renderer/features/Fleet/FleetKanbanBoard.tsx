...
import type { FleetColumn, FleetProjectId, FleetTicket, FleetTicketId } from '@/shared/types';

import { FleetKanbanCard } from './FleetKanbanCard';
import { FleetKanbanColumn } from './FleetKanbanColumn';
import { $fleetPipeline, $fleetTickets, fleetApi } from './state';

export const FleetKanbanBoard = memo(({ projectId }: { projectId: FleetProjectId }) => {
  const pipeline = useStore($fleetPipeline);
  const tickets = useStore($fleetTickets);

  const [activeTicket, setActiveTicket] = useState<{ ticket: FleetTicket; column: FleetColumn } | null>(null);
  ...
  const ticketsByColumn = useMemo(() => {
    const map: Record<string, FleetTicket[]> = {};
    if (!pipeline) {
      return map;
    }
  ...
  const handleDragStart = useCallback(
    (event: DragStartEvent) => {
      const ticketId = event.active.id as FleetTicketId;
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const ticketId = active.id as FleetTicketId;
      ...
      if (currentColumnId !== newColumnId) {
        fleetApi.moveTicketToColumn(ticketId, newColumnId);
      }
    },
    [tickets, pipeline]
  );
    ...
    >
      <div className="flex gap-3 h-full overflow-x-auto px-4 py-3">
        {pipeline.columns.map((column) => (
          <FleetKanbanColumn key={column.id} column={column} tickets={ticketsByColumn[column.id] ?? []} />
        ))}
      </div>
...
FleetKanbanBoard.displayName = 'FleetKanbanBoard';

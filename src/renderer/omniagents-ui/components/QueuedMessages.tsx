import type { QueuedMessage } from '../rpc/client'
    ...
    <div className="px-3 pt-2">
      <div className="rounded-md border border-bgCardAlt bg-bgCardAlt/60 p-2.5">
        <div className="flex items-center gap-2 text-xs text-textSubtle">
          ...
          <span><span className="text-brand">{items.length}</span> queued</span>
        </div>
        <ul className="mt-1.5 space-y-1">
          {items.map((item, idx) => (
            ...
            >
              <span className="mt-0.5 w-5 shrink-0 text-right tabular-nums text-textSubtle">
                {idx + 1}
              </span>
              <span className="min-w-0 flex-1 whitespace-pre-wrap break-words text-textPrimary">
                {item.content}
              </span>
              <button
                type="button"

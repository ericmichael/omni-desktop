...
export default function Orb({ hue = 0, hoverIntensity = 0.2, rotateOnHover = true, forceHoverState = false, animationSpeed = 1.0, noiseScale = 0.65, noiseAmplitude = 1.0, innerRadius = 0.6 }) {
  ...
  const frag = /* glsl */ `
    ...
    vec4 draw(vec2 uv) {
      ...
      float n0 = snoise3(vec3(uv * noiseScale, iTime * animationSpeed * 0.5)) * 0.5 + 0.5;
      float n0Scaled = n0 * noiseAmplitude;
      float r0 = mix(mix(innerRadius, 1.0, 0.4), mix(innerRadius, 1.0, 0.6), n0Scaled);
      float d0 = distance(uv, (r0 * invLen) * uv);
      float v0 = light1(1.0, 10.0, d0);
      v0 *= smoothstep(r0 * 1.05, r0, len);
      float cl = cos(ang + iTime * animationSpeed * 2.0) * 0.5 + 0.5;

      float a = iTime * animationSpeed * -1.0;
...
return;
    ...
    const update = t => {
      ...
      program.uniforms.animationSpeed.value = animationSpeedRef.current;
      program.uniforms.noiseScale.value = noiseScaleRef.current;
      program.uniforms.noiseAmplitude.value = noiseAmplitudeRef.current;
      program.uniforms.innerRadius.value = innerRadiusRef.current;

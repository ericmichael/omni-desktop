...
export function ArtifactPortalProvider({
  target,
  children,
}: {
...
export function MessageList({
  ...
  onReaction?: (type: 'like' | 'dislike', text?: string) => void;
  currentRunId?: string;
  ...
  const displayItems = useMemo(
    () => groupItems(visibleItems, currentRunId, !!thinking),
    [visibleItems, currentRunId, thinking]
  );
...
function InlineArtifact({
  ...
  const artifact = (
    <Artifact className={canPortal ? 'h-full w-full shadow-lg' : undefined}>
      <ArtifactHeader>
        ...
        <ArtifactActions>
          {copyable ? (
            <ArtifactAction tooltip={copied ? 'Copied!' : 'Copy'} icon={CopyIcon} onClick={handleCopy} />
          ) : null}
          <ArtifactAction
  ...
  if (canPortal) {
    ...
    return createPortal(
      <div className="absolute inset-0 z-20 flex bg-background/95 p-3">{artifact}</div>,
      portalTarget
    );
  }
  ...
  return (
    <div className="flex justify-start" data-artifact-id={item.artifact_id || undefined}>
      <div className="w-full min-w-0 max-w-full overflow-hidden">{artifact}</div>
    </div>
  );
}
  ...
  if (mode === 'image') {
    return (
      <img
        ...
        className={maximized ? 'mx-auto max-h-full max-w-full rounded' : 'max-w-full rounded'}
      />
    );
  }
...
function McpUiSurface({
  ...
  const inlineHtml = useMemo(() => resolveMcpUiHtml(payload.resource?.resource), [payload.resource?.resource]);
...
function ApprovalCard({
  ...
  if (isMcp && item.server_label) {
    headerSuffix = (
      <span className="font-mono text-xs text-warning/90">
        {' '}
        — {item.server_label}/{item.tool}
      </span>
    );
  } else if (
    (displayType === 'file_write' || displayType === 'diff') &&
    metaInner &&
    typeof metaInner.file_path === 'string' &&
    metaInner.file_path
  ) {
...
function tableView(metadata: any) {
  ...
  const cols: string[] = Array.isArray(t.columns)
    ? t.columns.map((c: any) => (c && typeof c === 'object' ? c.title : '')).filter(Boolean)
    : [];
  const rows: string[][] = Array.isArray(t.rows)
    ? t.rows
        .map((r: any) => (Array.isArray(r) ? r.map((e: any) => (e == null ? '' : String(e))) : []))
...
function commandView(metadata: any, plainOutput: string) {
  ...
  if (typeof meta?.was_truncated === 'boolean' && meta.was_truncated) {
    ...
    status.push(
      typeof charsTruncated === 'number' ? `${charsTruncated.toLocaleString()} chars truncated` : 'truncated'
    );
  }

...
const CodeBlockBody = memo(
  ({ tokenized, showLineNumbers, className }: { tokenized: TokenizedCode; showLineNumbers: boolean; className?: string }) => {
    const preStyle = useMemo(() => ({ backgroundColor: tokenized.bg, color: tokenized.fg }), [tokenized.bg, tokenized.fg])
...
export const CodeBlockContent = ({
  ...
  const syncTokens = useMemo(() => highlightCode(code, language as BundledLanguage) ?? rawTokens, [code, language, rawTokens])
...
export const CodeBlockLanguageSelectorTrigger = ({ className, ...props }: CodeBlockLanguageSelectorTriggerProps) => (
  <SelectTrigger className={cn('h-7 border-none bg-transparent px-2 text-xs shadow-none', className)} size="sm" {...props} />
)

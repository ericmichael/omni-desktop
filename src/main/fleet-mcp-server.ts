 ...
 * Fleet MCP Server
 *
 * Exposes fleet project-management tools over the MCP Streamable HTTP transport.
...
import type { FleetTicketId, FleetPipeline } from '@/shared/types';

// ---------------------------------------------------------------------------
//  Context interface — the FleetManager passes a narrow delegate so this
//  module stays decoupled from the full manager.
// ---------------------------------------------------------------------------

export type FleetMcpDelegate = {
  /** Return ticket data (or null) for the bound ticket. */
  getTicket: (ticketId: FleetTicketId) => {
    id: string;
    title: string;
    description: string;
    priority: string;
  ...
  getPipeline: (projectId: string) => FleetPipeline;

  /** Move a ticket to a column (by column id). */
  moveTicketToColumn: (ticketId: FleetTicketId, columnId: string) => void;

  /** Escalate — pause the run and notify the human. Returns a promise that resolves once the run is stopped. */
  escalate: (ticketId: FleetTicketId, message: string) => Promise<void>;
};

// ---------------------------------------------------------------------------
//  FleetMcpServer — one per ticket/sandbox
// ---------------------------------------------------------------------------
...
export class FleetMcpServer {
  ...
  constructor(
    private ticketId: FleetTicketId,
    private delegate: FleetMcpDelegate
  ) {
   ...
   * the Docker gateway (always allowed by the network isolation script).
   */
  containerUrl(): string {
  ...
  async start(): Promise<number> {
    this.transport = new StreamableHTTPServerTransport({
      sessionIdGenerator: undefined, // stateless — one transport per ticket
    });
    ...
    return new Promise<number>((resolve, reject) => {
      this.httpServer!.listen(0, '0.0.0.0', () => {
        ...
        if (addr && typeof addr === 'object') {
          ...
          console.log(`[FleetMcpServer] Listening on port ${this._port} for ticket ${this.ticketId}`);
          resolve(this._port);
        } else {
  ...
  async stop(): Promise<void> {
    ...
    this._port = null;
    console.log(`[FleetMcpServer] Stopped for ticket ${this.ticketId}`);
  }
}

...
import { parseFleetWorkflow } from './fleet-workflow';

describe('parseFleetWorkflow', () => {
  it('parses empty content as empty config with empty prompt', () => {
    const result = parseFleetWorkflow('');
    expect(result.config).toEqual({});
    expect(result.promptTemplate).toBe('');
  });

  it('parses content without frontmatter as prompt-only', () => {
    const result = parseFleetWorkflow('You are a supervisor agent.\n\nDo good work.');
    expect(result.config).toEqual({});
